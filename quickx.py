...
process=None
def runWithPlayer(srcDir, isSingle):
    global process
    # root
    ...
    if sublime.platform()=="osx":
        playerPath=quick_cocos2dx_root+"/player3.app/Contents/MacOS/player3"
            ...
            playerPath=quick_cocos2dx_root+"/../simulator/win32/WeiLeProjV3.exe"
    ...
    args.append("-file")
    args.append(quick_cocos2dx_root+"/../src/main.lua")

    accountJsonPath=path+"/../account.json"
    if os.path.exists(accountJsonPath):
        with open(accountJsonPath) as data_file:    
    ...
    #     f.close()
    #     args.append("-size")
    #     args.append(str(width)+"x"+str(height))
    # if process:
    #     try:
    #         process.terminate()
    #     except Exception:
...
class QuickxRunWithPlayerByFileCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        path="D:/SVN/WLGameHall/src"
        runWithPlayer(path, True)
...
class KillByProcessNameShellCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        os.system("taskkill /f /im " + "WeiLeProjV3.exe")

class QuickxRunTestWithPlayerByFileCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        path="D:/SVN/WLGameHall/src"
        runWithPlayer(path, False)
